...
class MainPage(BasePage):
    ...
    def go_to_register_page(self):
        search_field = self.find_element(MainPage.locator_register)
        search_field.click()

    def go_to_log_in_page(self):
        search_field = self.find_element(MainPage.locator_log_in)
        search_field.click()

    def go_to_profile_page(self):

        search_field = self.find_element(MainPage.locator_profile_button)
        search_field.click()

    def log_out(self):

        search_field = self.find_element(MainPage.locator_log_out)
        search_field.click()

    def go_to_search_result_page(self):

        search_field_1 = self.find_element(MainPage.locator_search_field)
        search_field_1.send_keys("Simple Computer")

        search_field_2 = self.find_element(MainPage.locator_search_button)
        search_field_2.click()

    def go_to_book_list_page(self):

        search_field = self.find_element(MainPage.locator_books_ref_button)
        search_field.click()


    def go_to_gift_list_page(self):

        search_field = self.find_element(MainPage.locator_giftcard_ref_button)
        search_field.click()